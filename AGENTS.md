...
├── toolbar.js    # vanilla-JS toolbar, no build step
...
`SKILL.md` tells Claude to: get the active tab via `claude-in-chrome`, `Read` `toolbar.js`, pass its contents to `javascript_tool` (the toolbar self-mounts via Shadow DOM), wait for the user's "done" signal (button on the toolbar *or* the word "done" in chat), then read `window.__avis.annotations` back and edit code. The toolbar captures React `_debugSource` (`src/file.tsx:42`) when available, falls back to component path, visible text, accessibility attributes, and a CSS selector — in that priority order.
...
- **Keep it small.** If you find yourself adding 200+ more LOC, ask whether the feature belongs in a separate skill.
